...
```{r bottom-temp-gom, fig.cap="GB and GOM annual bottom temperature anomalies."}
ecodata::bottom_temp %>%
...
```{r bottom-temp-gb, fig.cap="GB and GOM annual bottom temperature anomalies."}
ecodata::bottom_temp %>%
  dplyr::filter(EPU == "GB",
         Var == "bottom temp anomaly in situ") %>%
  dplyr::mutate(hline = 0) %>% 
...
```{r pp-weekly, fig.width = 8, fig.cap = "Weekly chlorophyll concentrations and primary productivity for 2020 in Georges Bank and Gulf of Maine are shown by the colored lines in the above figures. The long-term mean is shown in black and shading indicates +/- 1 sample SD."}
...
```{r weekly-phyto-size-gom, eval = T}
