...
```{r comdat-commercial-landings-gb, warning=F, fig.length = 24, fig.cap = "NEFMC managed species landings (red) and total commercial landings (black) by feeding guild in Gulf of Maine (left) and Georges Bank (right)."}
...
```{r comdat-commercial-landings-gom, warning=F, fig.length = 24, fig.cap = "NEFMC managed species landings (red) and total commercial landings (black) by feeding guild in Gulf of Maine (left) and Georges Bank (right)."}
...
```{r aquaculture-pa, fig.cap="Oyster aquaculture production in pieces for four New England states (Maine, Massachusettes, New Hampshire and Rhode Island)."}
 ecodata::aquaculture %>%
  dplyr::filter(Region == c("ME", "MA", "NH", "RI"))  %>% 
  dplyr::filter(!Value == "NA") %>% 
  dplyr::mutate(Time = as.integer(Time), 
